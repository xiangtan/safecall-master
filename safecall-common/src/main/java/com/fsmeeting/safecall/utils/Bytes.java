...
public class Bytes {
	 ...
	 * OutputStream os = new DeflaterOutputStream(bos); try { os.write(bytes); }
	 * finally { os.close(); bos.close(); } return bos.toByteArray(); }
	 */
	/*
		*//**
			 * unzip.
			 ...
			 * UnsafeByteArrayInputStream(bytes); UnsafeByteArrayOutputStream
			 * bos = new UnsafeByteArrayOutputStream(); InputStream is = new
			 * InflaterInputStream(bis); try { IOUtils.write(is, bos); return
			 * bos.toByteArray(); } finally { is.close(); bis.close();
			 * bos.close(); } }
			 */
